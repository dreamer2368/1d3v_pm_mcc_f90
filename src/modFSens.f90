module modFSens
	...
	type, extends(PM1D) :: FSens
        type(phaseSpaceMesh), allocatable :: psM(:)
!		real(mp) :: Lv, dv
!		integer :: ngv
...
contains
	...
	subroutine buildFSens(dpm,pm,Lv,Ngv,NInject,NLimit)
        ...
        allocate(dpm%psM(pm%n))
		do i=1,pm%n
			call dpm%p(i)%buildSpecies(pm%p(i)%qs,pm%p(i)%ms)
            call dpm%psM(i)%buildPhaseSpaceMesh(pm%L,Lv(i),pm%ng,Ngv(i),pm%m%dx)
		end do
        ...
        if( dpm%scheme.eq.INJECTION ) then
            do i=1,pm%n
    		call dpm%psM(i)%createDistribution(dpm%a(i),dpm%NInject,dpm%xp_inject,dpm%vp_inject,	&
	    									    dpm%g_inject,dpm%gv_inject,dpm%frac_inject)
	    	call dpm%psM(i)%createDistribution(dpm%a(i),dpm%NLimit,dpm%xp_remesh,dpm%vp_remesh,	    &
	...
	subroutine destroyFSens(this)
        ...
        do i=1,this%n
            call this%psM(i)%destroyPhaseSpaceMesh
        end do
        deallocate(this%psM)
...
!	subroutine FSensSourceTerm(dpm,psM,qs,ms,nk,fsr)					!Take v-derivative, Multiply dE
!		class(FSens), intent(inout) :: dpm
!        class(phaseSpaceMesh), intent(inout) :: psM
!		real(mp), intent(in) :: qs,ms
!		integer, intent(in), optional :: nk
!		type(recordData), intent(in), optional :: fsr
!		integer :: ng,ngv,kr,i,j
...
!        do j=1,dpm%n
!		    do i=1,2*ngv+1
!		    	dpm%psM(j)%J(:,i) = dpm%psM(j)%Dvf(:,i)*E_temp 
!            end do
!        end do
!
!		!Multiply dt
...
!	end subroutine
	...
	subroutine InjectSource(this,Lx,Lv,p,J)
		class(FSens), intent(inout) :: this
		...
		call SourceAssign(J,Lx,Lv,this%g_inject,this%gv_inject,this%frac_inject,spwt0)
...
!	end subroutine
	...
	subroutine Redistribute(this,p,Lv)  !periodic BC only!!
		class(FSens), intent(inout) :: this
		type(species), intent(inout) :: p
        real(mp), intent(in) :: Lv
